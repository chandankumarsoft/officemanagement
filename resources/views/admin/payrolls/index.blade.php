...
<div class="card">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3 px-4">
        <h5 class="mb-0 fw-bold"><i class="bi bi-cash-stack me-2 text-success"></i>Payroll Records</h5>
        <a href="{{ route('admin.payrolls.create') }}" class="btn btn-primary btn-sm">
            <i class="bi bi-plus-circle me-1"></i>Generate Payroll
        </a>
    </div>
    {{-- Filter bar --}}
    <div class="filter-bar">
        <form method="GET" action="{{ route('admin.payrolls.index') }}" class="d-flex flex-wrap gap-2 w-100">
            ...
            <select name="month" class="form-select form-select-sm" style="max-width:140px">
            ...
            <select name="year" class="form-select form-select-sm" style="max-width:110px">
            ...
            <a href="{{ route('admin.payrolls.index') }}" class="btn btn-outline-secondary btn-sm">Clear</a>
            @endif
        </form>
    </div>
            ...
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="fw-semibold small">{{ $payroll->employee->full_name ?? '—' }}</td>
                        <td class="small">
                        ...
                        <td class="text-end pe-4">
                            <a href="{{ route('admin.payrolls.show', $payroll) }}" class="btn btn-sm btn-outline-info">
                                <i class="bi bi-eye"></i>
                            </a>
                            @if($payroll->status !== 'paid')
                            <form action="{{ route('admin.payrolls.markAsPaid', $payroll) }}" method="POST" class="d-inline">
                                @csrf
                            ...
                            <button type="button" class="btn btn-sm btn-outline-danger"
                                ...
                                data-action="{{ route('admin.payrolls.destroy', $payroll) }}"
                                data-message="Delete payroll for '{{ $payroll->employee->full_name ?? 'this employee' }}' ({{ DateTime::createFromFormat('!m', $payroll->pay_month)->format('F') }} {{ $payroll->pay_year }})? This cannot be undone."
                                title="Delete">
                    ...
                    @empty
                    <tr><td colspan="9" class="text-center text-muted py-5">No payroll records found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
